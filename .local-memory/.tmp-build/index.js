...
// .local-memory/src/projection/engine.ts
...
class ProjectionEngine {
  ...
  auditRepo;
  config;
  constructor(memoryRepo, auditRepo, config = {}) {
    this.memoryRepo = memoryRepo;
    this.auditRepo = auditRepo;
  ...
  async rebuild(context) {
    ...
    try {
      ...
      const semanticMemories = await this.memoryRepo.findByLayer("semantic", "active");
      const coreMemories = await this.memoryRepo.findByLayer("core", "active");
      const allMemories = [...semanticMemories, ...coreMemories].sort((a, b) => a.id.localeCompare(b.id));
      console.log(`[ProjectionEngine] Found ${allMemories.length} memories to project ` + `(${semanticMemories.length} semantic, ${coreMemories.length} core)`);
      ...
      if (context) {
        await this.auditRepo.record("project", "projection", "full_rebuild", { projected: projectedCount, errors: errors.length }, context);
      }
      return {
  ...
  async projectMemory(memory, context) {
    ...
    if (context) {
      await this.auditRepo.record("project", "memory", memory.id, { filePath, layer: memory.layer, type: memory.type }, context);
    }
  }
  async removeProjection(memoryId, context) {
    const memory = await this.memoryRepo.findById(memoryId);
    if (!memory) {
      throw new Error(`Memory not found: ${memoryId}`);
    }
    ...
    if (context) {
      await this.auditRepo.record("project", "memory", memoryId, { action: "remove", filePath }, context);
    }
  }
  async updateProjection(memoryId, context) {
  ...
  getFilePath(memory, mapping) {
    ...
    if (mapping.strategy === "per_item" && mapping.pattern) {
      ...
      const filename = mapping.pattern.replace("{id}", memory.id.substring(0, 8)).replace("{slug}", slug).replace("{date}", date);
      return join(this.config.projectionRoot, targetDir, filename);
    }
    throw new Error(`Invalid mapping strategy for type: ${memory.type}`);
  }
  generateMarkdown(memory) {
...
${memory.content}`;
  ...
  slugify(text) {
    return text.toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").substring(0, 50).replace(/-+$/, "");
  }
...
// .local-memory/src/cleanup/service.ts
var DEFAULT_CONFIG6 = {
...
class CleanupService {
  ...
  projectionEngine;
  config;
  constructor(db, memoryRepo, auditRepo, projectionEngine, config = {}) {
    this.db = db;
    this.memoryRepo = memoryRepo;
    this.auditRepo = auditRepo;
  ...
  async runFullCleanup(context) {
    ...
    console.log(`[CleanupService] Cleanup complete: ${totalProcessed} processed, ` + `${totalArchived} archived, ${totalDeleted} deleted`);
    return report;
  }
  async decayEpisodic(context) {
    const result = {
      task: "decay_episodic",
      processed: 0,
      archived: 0,
        ...
        if (memoryDate < cutoff) {
          try {
            if (!this.config.dryRun) {
              ...
              if (context) {
                await this.auditRepo.record("update", "memory", memory.id, { action: "archive", reason: "retention_policy", age: memoryDate.toISOString() }, context);
              }
            }
            result.archived++;
        ...
        if (memoryDate < cutoff) {
          try {
            if (!this.config.dryRun) {
              await this.memoryRepo.updateStatus(memory.id, "archived");
              if (context) {
                await this.auditRepo.record("update", "memory", memory.id, { action: "archive", reason: "working_cleanup", age: memoryDate.toISOString() }, context);
              }
            }
            result.archived++;
          } catch (error) {
  ...
  async cleanupEmbeddings(context) {
    const result = {
    ...
    try {
      ...
      const rows = this.db.query(`SELECT e.id, e.memory_id FROM embeddings e
         JOIN memories m ON e.memory_id = m.id
         WHERE m.status IN ('archived', 'forgotten', 'reverted')
         AND m.updated_at < ?`).all(cutoff.toISOString());
      for (const row of rows) {
        result.processed++;
        try {
          if (!this.config.dryRun) {
            ...
            if (context) {
              await this.auditRepo.record("delete", "embedding", row.id, { memoryId: row.memory_id, reason: "stale_cleanup" }, context);
            }
          }
          result.deleted++;
        } catch (error) {
  ...
  async repairOrphanedRelations(context) {
    const result = {
    ...
    try {
      const rows = this.db.query(`SELECT r.id, r.source_id, r.target_id FROM memory_relations r
         JOIN memories m ON r.target_id = m.id
         WHERE m.status IN ('archived', 'forgotten', 'reverted')
         AND r.status = 'active'`).all();
      for (const row of rows) {
        result.processed++;
        try {
          if (!this.config.dryRun) {
            this.db.run("UPDATE memory_relations SET status = ? WHERE id = ?", ["inactive", row.id]);
            if (context) {
              await this.auditRepo.record("update", "memory_relation", row.id, { action: "deactivate", reason: "orphaned_target", targetId: row.target_id }, context);
            }
          }
          result.archived++;
        } catch (error) {
  ...
  async cleanupStaleProjection(context) {
    const result = {
    ...
    try {
      ...
      const toCleanup = nonActiveMemories.filter((m) => m.status !== "active" && m.status !== "new");
  ...
  async archiveOldAudit(context) {
    ...
    try {
      ...
      const count = this.db.query("SELECT COUNT(*) as count FROM audit_log WHERE created_at < ?").get(cutoff.toISOString());
      if (count.count > 0) {
  ...
  async cleanupMemory(memoryId, context) {
    const memory = await this.memoryRepo.findById(memoryId);
    if (!memory) {
      throw new Error(`Memory not found: ${memoryId}`);
    }
    ...
    this.db.run("DELETE FROM embeddings WHERE memory_id = ?", [memoryId]);
    this.db.run("UPDATE memory_relations SET status = ? WHERE source_id = ? OR target_id = ?", ["inactive", memoryId, memoryId]);
...
class MemoryCoreService {
  ...
  async initializeServices() {
    ...
    this.cleanupService = new CleanupService(this.db, this.memoryRepo, this.auditRepo, this.projectionEngine);
  }
  async health() {
    const checks = {
  ...
  ensureDirectories() {
    const dirs = [
      ...
      join2(this.config.runtimeRoot, "logs"),
      join2(this.config.runtimeRoot, "cache")
    ];
  ...
  async checkProjection() {
    ...
    try {
      const testFile = join2(this.config.projectionRoot, ".health");
...
async function handlePromotionEvaluate(req, deps) {
  try {
    ...
    return Response.json({ error: "Promotion evaluation failed", message: String(error) }, { status: 500 });
  }
}
async function handlePromotionPromote(req, deps) {
  try {
    ...
    return Response.json({ error: "Promotion failed", message: String(error) }, { status: 500 });
  }
}
async function handleRelationCreate(req, deps) {
  try {
    const input = await req.json();
    const result = await deps.service.getRelationEngine().createRelation(input, { actor: "api" });
    return Response.json(result);
  } catch (error) {
    return Response.json({ error: "Relation creation failed", message: String(error) }, { status: 500 });
  }
}
async function handleRelationGet(req, deps) {
  try {
    ...
    return Response.json({ error: "Relation query failed", message: String(error) }, { status: 500 });
  }
}
async function handleRelationDeactivate(req, deps) {
  try {
    ...
    return Response.json({ error: "Relation deactivation failed", message: String(error) }, { status: 500 });
  }
}
...
function buildRoutes(deps) {
  ...
  return {
    ...
    "/api/promotions/evaluate": {
      POST: (req) => handlePromotionEvaluate(req, opsDeps)
    },
    "/api/promotions/promote": {
      POST: (req) => handlePromotionPromote(req, opsDeps)
    },
    "/api/relations": {
      POST: (req) => handleRelationCreate(req, opsDeps)
    },
    "/api/relations/:memoryId": {
      GET: (req) => handleRelationGet(req, opsDeps)
    },
    "/api/relations/:id": {
      DELETE: (req) => handleRelationDeactivate(req, opsDeps)
    }
  };
}
...
async function runPromote(options) {
  ...
  try {
    await service.initialize();
    const result = await service.getPromotionEngine().promote(options.memoryId, { actor: "cli" }, options.force ?? false);
    console.log(JSON.stringify(result, null, 2));
...
async function runRelations(options) {
  if (!options.memoryId) {
    console.error("Error: --memory-id is required");
    return 1;
  }
...
async function runProjectionRebuild(options) {
  const service = new MemoryCoreService(options);
  try {
    await service.initialize();
...
async function runProjectionVerify(options) {
  const service = new MemoryCoreService(options);
  try {
    await service.initialize();
